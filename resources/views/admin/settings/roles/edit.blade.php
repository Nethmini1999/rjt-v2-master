...
<div class="card border-left-danger">
    <div class="card-body">
        {{ Form::open(['url' => '/admin/settings/update-roles/', 'id'=>'editRecordFrom']) }}
            <input type="hidden" name="id" value="{{$role->id}}" />
            <div class="row mt-3">
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="Name">Name</label>
                        <input type="input" class="form-control" id="Name" name="name" value="{{$role->name}}" readonly>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="Description">Description</label>
                        <input type="input" class="form-control" id="Description" name="description" value="{{$role->description}}" required>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label>&nbsp;</label>
                        <button type="submit" class="btn btn-primary btn-grey-overlay d-block">Update</button>
                    </div>
                </div>
            </div>
            <div class="row">
                                    ...
                                    <tr>
                                        <td width="15px">
                                            <div class="checkbox checkbox-switch">
                                                <label>
                                                    <input type="checkbox" value="{{$permission['id']}}" name="permissions[{{$permission['id']}}]" @if(isset($role_permissions[$permission['id']])) checked @endif>
                                                </label>
                                            </div>
                                        </td>
                                        <td>                                    
